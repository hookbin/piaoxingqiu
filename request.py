...
def requestsGet(url, headers):
    ...
    print("<<< " + url + " " + str(round(time.time() - start, 3) * 1000) + "ms")
    return result
...
def get_sessions(show_id) -> list | None:
    ...
    response = requestsGet(url=url, headers=headers)
...
def get_seat_plans(show_id, session_id) -> list:
    ...
    url = (
        "https://m.piaoxingqiu.com/cyy_gatewayapi/show/pub/v5/show/"
        + show_id
        + "/session/"
        + session_id
        + "/seat_plans"
...
def get_deliver_method(
    show_id, session_id, seat_plan_id, price: int, qty: int, token
) -> str:
    headers = {
        "User-Agent": "Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.75 Mobile Safari/537.36",
        "Content-Type": "application/json",
    ...
    response = requestsPost(url=url, headers=headers, data=data)
    if response["statusCode"] == 200:
